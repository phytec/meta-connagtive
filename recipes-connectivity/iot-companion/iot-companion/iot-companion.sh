...
if [ -s $AwsClientConfig.sha256 ]; then
	if ! sha256sum --check "$AwsClientConfig.sha256"; then
		...
		echo "Creating new jq dump file as new awsclient config file found"
	...
	echo "No stored hash for '$AwsClientConfig', creating hash file"
	sha256sum "$AwsClientConfig" > "$AwsClientConfig.sha256"

	echo "Creating jq dump file for first time"
